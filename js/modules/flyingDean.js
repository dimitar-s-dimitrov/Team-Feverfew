...
define("flyingDean", function () {
    ...
    $flyingDeanTemplates = $("#flying-dean-templates-wrapper").children();
    templatesCount = $flyingDeanTemplates.length;

    // Generate and return flying Dean with random type
    function generateRandomType() {
        var randomNumber = Math.random() * templatesCount;
        var randomTemplate = $flyingDeanTemplates.eq(Math.floor(randomNumber));
    ...
    function animateElement(flyingDean, movesLeft) {
        var newRandomPosition = generateRandomPosition(flyingDean.width, flyingDean.height);

        flyingDean.$element.animate(newRandomPosition, flyingDean.speed, function () {
            if (--movesLeft > 0) {
                animateElement(flyingDean, movesLeft);
    ...
    // Constructor
    function FlyingDean(defaultSpeed) {
        var elementTop, elementLeft;

        // Create flying Dean with random type
    ...
    FlyingDean.prototype = {
        ...
        // Reinitializes the element to new starting position and stops any animation running
